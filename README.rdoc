...
== Description
Uses ruby-serialport to connect to a standard serial GPS unit and parses the NMEA sentences into an easy to read hash.

=== NMEA 
Currently it only parses the following NMEA sentenses:
* $GPGGA
* $GPRMC
* $GPGLL
* $GPRMA
...
== Date and Time
GPS modules provide the current time and date strings in the UTC timezone. 
This date_time method which is the DateTime object version, still in the UTC timezone.
...
=== Print Latitude
A program that prints latitude data to the console as soon as it's available.
